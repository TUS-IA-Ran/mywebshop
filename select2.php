...
<?php
    ...
    echo "<h1><marquee width='60%'>ファッション小物関連商品</marquee></h1>";
    ...
    $pdo = connect_mysql();
    if(isset($pdo)){
        $sql = "SELECT * FROM mygoods where category='ファッション小物'";
        $result = $pdo->query($sql);
        while($row = $result->fetch(PDO::FETCH_ASSOC)){
            echo "<div align = 'center'>";
            echo "商品名: " . $row["name"] . "<br>";
            echo "<a href='mycago.php?item=" . $row["id"] . "'>" . "<img src='img/" . $row["id"] . ".png' width='150' height='120'></a><br>";
